public class YahtzeeScorecard {
    ...
    public boolean markThreeOfAKind(int var1, int var2, int var3, int var4, int var5) {
        if (threeKind != 1){
            ...
            if (sorter.getFirst() == sorter.getThird() || sorter.getThird() == sorter.getFifth()) {
                threeKind = sorter.getThird() * 3;
            } else if (sorter.getSecond() == sorter.getFourth()){
                threeKind = sorter.getSecond() * 3;
            } else {
                threeKind = 0;
            }
            return true;
    ...
    public boolean markFourOfAKind(int var1, int var2, int var3, int var4, int var5) {
        if (fourKind != 1){
            ...
            if (sorter.getFirst() == sorter.getFourth()) {
                fourKind = sorter.getFirst() * 4;
            } else if (sorter.getSecond() == sorter.getFifth()){
                fourKind = sorter.getSecond() * 4;
            } else {
                fourKind = 0;
            }
            return true;
    ...
    public boolean markSmallStraight(int var1, int var2, int var3, int var4, int var5) {
        if (smStraight != 1){
            ...
            YahtzeeSortDice sorter = new YahtzeeSortDice(var1, var2, var3, var4, var5);
            if (sorter.getFirst() == sorter.getFourth() + 3 || sorter.getSecond() == sorter.getFifth() + 3) {
                smStraight = 30;
            } else {
                smStraight = 0;
            }
    ...
    public boolean markLargeStraight(int var1, int var2, int var3, int var4, int var5) {
        if (lgStraight != 1){
            ...
            YahtzeeSortDice sorter = new YahtzeeSortDice(var1, var2, var3, var4, var5);
            if (sorter.getFirst() == sorter.getFifth() + 4) {
                lgStraight = 40;
            } else {
                lgStraight = 0;
            }
    ...
    /* getUpperTotal TODO:
    Add up the upper total (aka ones, twos, threes, fours, fives, and sixes) and return the sum. If the score is
    63 or more, remember to add a 35 point bonus! */
    ...
    */
    public int getLowerTotal() {
        return threeKind + fourKind + fullHouse + smStraight + lgStraight + yahtzee + chance + yahtzeeBonus * 100;
    }
